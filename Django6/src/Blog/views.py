...
def searchP(request):
    ...
    #type :'0' 제목 검색
    if type=='0':
        #모델클래스.objects.fiter(): 특정조건을 만족하는 모든객체 추출
        #filter,get,exclude에 조건을 넣을때 (모델크랠스의 변수_명령어=값)
        #형태로 넣음
        #contains : 우변값이 해당 변수에 포함되있는 객체를 모두 추출
        list = Post.objects.fiter(headline__contains=q)
        return render(request, "Blog/templates/searchP.html",
                    {'list' : list} )
    elif type=='1': #type :'1' 글쓴이 검색
         user = User.objects.get(username = q )
         list = Post.objects.filter(author = user)
         return rendr (request, "Blog/templates/searchP.html",
                       {'list': list})
